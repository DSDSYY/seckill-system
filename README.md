...
- **活动与商品**：秒杀活动配置、库存预热、活动时间窗校验
- **抢购下单**：Redis Lua 原子预扣库存，防超卖
- **一人一单**：SETNX 资格校验 + 数据库唯一索引兜底
- **接口限流**：单用户令牌桶（Redis + Lua）
- **异步落库**：RabbitMQ 异步下单，接口快速返回
- **超时取消**：TTL + 死信队列实现 15 分钟未支付自动取消并回滚库存
- **登录鉴权**：JWT，支持 mock 微信登录便于本地联调
- **小程序端**：`miniprogram/` 提供完整演示页面与接口封装
